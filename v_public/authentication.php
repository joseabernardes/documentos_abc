...
<html>
    ...
    <body>
        ...
        <?php
        if (!$loggedIn) {
            if ($added) { // terminou de fazer o registo
                ...
                <main id="auth">
                    ...
                    <form id="registar" class="auth-form" method="post" action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>" enctype="multipart/form-data">
                        ...
                        <input value="<?= $input['countryR'] ?>" class="<?= array_key_exists('countryR', $errors) ? INPUT_CLASS_ERROR_NAME : '' ?>" required id="countryR" type="text" placeholder="Pais" name="countryR" maxlength="50">
